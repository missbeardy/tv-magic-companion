...
### [~] T1.15 Voicemail fallback — recover the recordings CloudMailin rejects (added 11-08-2026, owner report — voicemails arriving inconsistently) — deployed v1.1.174 13-08-2026, cron green, awaiting live voicemail UAT

- **Why:** This is a live lead-loss bug, not a feature. Measured from two real samples the owner supplied: CloudMailin returns `552 Message size exceeds the allowed size for this account (524288)` on the forwarded voicemail. 3CX records **PCM mono 8 kHz 16-bit = 16 KB/sec**, and base64 inflates ~33%, so the ceiling is ~393 KB of audio — **roughly 25 seconds of speech**. The sample bounced at 26.6s (425,646 bytes → ~554 KB encoded). Every voicemail longer than that has been silently dropped: no lead, no callback, no trace in the app. A useful voicemail is usually longer than 25 seconds, so this is plausibly the majority of them. The audio cannot be salvaged from the bounce (21 KB DSN, headers only) — but the original is still in the Gmail mailbox.
- **Spec:** The voicemail branch of `api/inbound-email.ts` moves to a transport-neutral `api/_lib/processVoicemail.ts`; the CloudMailin webhook and a new IMAP poller both call it, so the two cannot drift. Everything downstream is reused unchanged (`extractVoicemailMetadata`, `findRecentLeadByPhone`, `enrichLeadFromVoicemailTranscript`, `insertRawFirstLead`, `processInboundLead` + hookback). **IMAP rather than the Gmail API** because the mailbox is a personal `@gmail.com`: restricted Gmail scopes on an unverified external app sit in "Testing" status, where refresh tokens expire every 7 days and the poller would die weekly. An app password has no expiry and no Google review, and IMAP's BODYSTRUCTURE means the server identifies the WAV part — `downloadMany` returns decoded bytes, so there is no MIME parsing at all. One new dependency (`imapflow`). **Recordings are now stored** in a private `lead-voicemails` bucket at `{org_id}/{uuid}.wav` (the lead-photos RLS shape) and played inline — they are plain PCM WAV, so no transcoding is needed.
- **Idempotency is the whole risk.** Gmail's auto-forward preserves Message-ID — confirmed, the DSN's `In-Reply-To` carries the original's `<oiXbIzArSCCjFWdeIG7Slw@geopod-ismtpd-14>` — so both transports derive the same key. `lead_voicemails.rfc_message_id` is UNIQUE and **the insert is the claim**: a 5-minute poll can overlap the instant webhook, so a SELECT-then-INSERT would leave a race window; the loser catches 23505 and backs off. Without it the second pass reaches `findRecentLeadByPhone` and logs a spurious `missed_call_again` — the same notification-noise class as the prod bug fixed in v1.1.167. The row is also written **before** Whisper runs, so a transcription failure now leaves a playable recording instead of "check 3CX manually".
- **Org routing:** one mailbox = one org via `VOICEMAIL_MAILBOX_ORG_ID`; unset ⇒ `not_configured` and nothing is touched. The poller cannot infer an org — a polled message has no CloudMailin plus-tag (`+default` exists only on the forwarded copy) and its `To: "166"` is a 3CX extension, which `looksLikePhoneNumber` rightly refuses to treat as a DID.
- **No new feature switch** — the existing `inbound_calls` gates both paths, and `dd19` (cut the catalog to twelve) is open backlog; the poller is gated on env-var presence instead. **No new function file** — `?action=voicemail-poll` on the `inbound-email` hub (11/12 slots), rewritten from `/api/cron/voicemail-poll`, scheduled from GitHub Actions every 5 min since Vercel crons are daily-only on Hobby. `api/inbound-email.ts` gains `maxDuration: 60`; it had been running Whisper on the 10s default, so the existing CloudMailin path was likely timing out intermittently too.
- **Done when:** a >25s voicemail creates a lead within 5 minutes with a playable recording; a <25s voicemail still arrives instantly via CloudMailin, now also with a recording; the same voicemail seen by both paths yields one lead, one row and **zero** `missed_call_again` events; and a technician in another org cannot fetch the storage path.
- **Shipped 13-08-2026 (v1.1.174):** migration applied to prod, code on `main` at `fa1dfdc`, cron registered and green on its own schedule — `{"success":true,"examined":0,...}`, which proves Actions → auth → config → IMAP → nested folder open → search. No Gmail filter was needed; the mailbox already labelled 3CX mail. The poller trusts that label rather than matching 3CX's subject wording (which would stop silently on a reword), with `looksLikeVoicemailNotification` guarding against a stray labelled email creating a junk lead.
- **Two things bit during rollout, both worth remembering.** (1) **A Gmail label's IMAP path is its display name, nested with `/` — not the hyphenated slug in Gmail's URL bar.** The label is `TVMagic Sales Lead/VoiceMail Lead`, we configured `tvmagic-sales-lead-voicemail-lead`, and the first prod run 500'd on `getMailboxLock`. The poller now lists available mailbox paths in that error so it self-diagnoses. (2) **"Skip the Inbox" does not break a Gmail forward** — archiving and forwarding are independent; an earlier note claiming otherwise was wrong. The setting that *would* break this is account-level forwarding set to *"delete Gmail's copy"*, leaving no original to poll.
- **Backlog deliberately suppressed:** the label held 93 messages (21 inside the 14-day lookback), all already worked manually by the owner. Every message was marked with the `fieldbourneVoicemailDone` keyword before go-live, or the first runs would have created ~21 stale leads plus a manager notification each. All 93 marked rather than just the window, so raising `LOOKBACK_DAYS` cannot resurrect them; removing that label from a message re-queues it, which is the manual re-process switch.
- **Remaining:** live UAT — a >30s voicemail must produce a lead with playable audio within 5 minutes, and a ~10s one must still arrive instantly via CloudMailin with audio attached. That is the first real exercise of the storage-before-transcribe path, Whisper, and the new bucket's RLS. Also still open: `VOICEMAIL_MAILBOX_ORG_ID` is set on the **Preview** scope and should be removed — preview shares the *same live mailbox*, and with credentials now known-good only the dev-Supabase FK error stops a preview poll marking real messages handled.
- **Later (not this item):** owner chose fallback-only so short voicemails stay instant — worth revisiting after a week of traffic, because if the poller handles the majority then retiring the CloudMailin voicemail branch collapses this to one code path. Loss monitoring is also open: if the poller dies, voicemails silently stop again; the DSN's `In-Reply-To` makes bounce-vs-`lead_voicemails` reconciliation possible.
