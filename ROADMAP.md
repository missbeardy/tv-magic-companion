...
### [~] T1.15 Voicemail fallback — recover the recordings CloudMailin rejects (added 11-08-2026, owner report — voicemails arriving inconsistently) — built, awaiting migration + mailbox setup
...
- **Remaining:** owner to apply `supabase/migrations/20260811160000_lead_voicemails.sql` to prod and set the `VOICEMAIL_*` env vars incl. a Google App Password (requires 2FA). **No Gmail filter needed** — the mailbox already labels 3CX voicemail mail `tvmagic-sales-lead-voicemail-lead`, now the configured default (confirmed 13-08-2026). The poller therefore trusts the label rather than matching 3CX's subject wording, which would have stopped silently if they ever reworded it; a content guard (`looksLikeVoicemailNotification`) stops a stray labelled email with an audio attachment from creating a junk lead. Reuses the existing `PLATFORM_URL` + `CRON_SECRET` secrets. Only mailbox setting that would break this: account-level forwarding set to *"delete Gmail's copy"*, leaving no original to poll.
