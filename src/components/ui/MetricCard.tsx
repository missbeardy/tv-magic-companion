...
export default function MetricCard({
  ...
  if (variant === 'hero') {
    return (
      <article
        className={`card p-5 border-l-4 h-full ${className}`}
        style={{
  ...
  if (variant === 'compact') {
    return (
      <article className={`rounded-xl border border-gray-200 bg-white px-3 py-2.5 ${className}`}>
  ...
  return (
    <article className={`card p-4 h-full ${className}`}>
