...
export default function TaskBoardPage() {
  ...
  async function fetchTasks() {
    ...
    if (!error && data) {
      ...
      const filtered = (data as Task[]).filter(task => {
        ...
        if (task.visibility === 'me_and_nick' && user?.id === NICK_USER_ID) return true
        return false
      })
      setTasks(filtered)
    }
  ...
  async function handleCreate() {
    ...
    await supabase.from('task_items').insert(
      validItems.map(label => ({ task_id: taskData.id, label: label.trim() }))
    )
  ...
  const visibilityLabel = (v: string) => {
    ...
    if (v === 'me_and_nick') return 'Me & Nick'
    return 'Everyone'
  }
