...
export async function runContactFollowUpCron(
  ...
  await processContactFollowUpRollovers(
    ...
    async (lead) => {
      ...
      // Stamping first also means a push failure can't leave the lead in a re-notify loop.
      const { error: stampError } = await supabase
        .from('leads')
        .update({ last_follow_up_reminder_at: new Date(nowMs).toISOString() })
        .eq('id', lead.id)
      if (stampError) {
        result.errors.push(`${lead.id} reminder stamp: ${stampError.message}`)
        return
      }
